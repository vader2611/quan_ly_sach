...
public class SachDAO {
    ...
    public static final String SQL_SACH ="CREATE TABLE Sach (maSach text primary key, maTheLoai text, " +
            "tensach text," +
    ...
    public Sach checkBook(String strPrimaryKey){
        ...
        String[] selectionArgs = {strPrimaryKey};
        Cursor c = null;
        try{  c = db.query(TABLE_NAME, columns, selection, selectionArgs, null, null, null);
            c.moveToFirst();
            while (c.isAfterLast()==false){
                s.setMaSach(c.getString(0));
                s.setMaTheLoai(c.getString(1));
